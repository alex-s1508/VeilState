...
ns.Modules.DruidSettings = {}

-- [[ UI BUILDERS ]] ----------------------------------------------------------

local G = ns.Modules.Settings and ns.Modules.Settings.Get or function(k, d) return ns.db and ns.db[k] or d end
local GetColor = ns.Modules.Settings and ns.Modules.Settings.GetColor or function(k, dr, dg, db) local c = ns.db and ns.db[k]; if not c then return dr or 0.055, dg or 0.027, db or 0.749 end; return c.r or dr, c.g or dg, c.b or db end

-- [[ PROWL SETTINGS ]] -------------------------------------------------------
local function AddProwlSettings(cat, prefix)
    ...
    SettingsLib:CreateInput(cat, {
        ...
        default = DefaultFor(K("customText"), "PROWLED"),
        get = function() return G(K("customText"), DefaultFor(K("customText"), "PROWLED")) end,
    ...
    SettingsLib:CreateColorOverrides(cat, {
        ...
        getColor = function(key) local def = ns.Defaults and ns.Defaults[key] or { r = 0.055, g = 0.027, b = 0.749 }; return GetColor(key, def.r, def.g, def.b) end,
        getDefaultColor = function(key) local def = ns.Defaults and ns.Defaults[key] or { r = 0.055, g = 0.027, b = 0.749 }; return def.r, def.g, def.b end,
...
-- [[ INITIALIZATION ]] -------------------------------------------------------
-- ============================================================================
function ns.Modules.DruidSettings.Init(root)
    if not ns.IsDruid then return end

    -- Prowl Category
    local prowlCat = SettingsLib:CreateCategory(root, ns.L and ns.L.Prowl or "Prowl", false)
    SettingsLib:CreateHeader(prowlCat, { name = ns.L and ns.L.Management or "Management" })
    SettingsLib:CreateCheckbox(prowlCat, {
        key = "prowlEnabled", name = ns.L and ns.L.EnableOnProwl or "Enable on Prowl", default = true,
        get = function() return G("prowlEnabled", true) end,
        set = function(v) ns.db.prowlEnabled = v; if ns.UpdateState then ns.UpdateState(true) end end,
    })
    SettingsLib:CreateCheckbox(prowlCat, {
    ...
    SettingsLib:CreateCheckbox(prowlCat, {
        key = "prowlOnlyInstances", name = ns.L and ns.L.OnlyInInstances or "Only in instances", default = false,
        get = function() return G("prowlOnlyInstances", false) end,
        set = function(v) ns.db.prowlOnlyInstances = v; if ns.UpdateState then ns.UpdateState(true) end end,
    })
    local cFilterSection = SettingsLib:CreateExpandableSection(prowlCat, { name = ns.L and ns.L.InstanceFilters or "Instance Filters", expanded = false })
    SettingsLib:CreateCheckbox(prowlCat, {
        key = "prowlDisableInDungeons", name = ns.L and ns.L.DisableInDungeons or "Disable in Dungeons", default = false,
        get = function() return G("prowlDisableInDungeons", false) end,
        set = function(v) ns.db.prowlDisableInDungeons = v; if ns.UpdateState then ns.UpdateState(true) end end,
        parentSection = cFilterSection,
    })
    SettingsLib:CreateCheckbox(prowlCat, {
        key = "prowlDisableInRaids", name = ns.L and ns.L.DisableInRaids or "Disable in Raids", default = false,
        get = function() return G("prowlDisableInRaids", false) end,
        set = function(v) ns.db.prowlDisableInRaids = v; if ns.UpdateState then ns.UpdateState(true) end end,
        parentSection = cFilterSection,
    })
    AddProwlSettings(prowlCat, "prowl")
end
