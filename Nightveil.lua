...
local function IsChannelUsable(channel)
    ...
    -- Group channels: only usable if in a group with others
...
local function SendChat(channel, msg)
    ...
    -- Use modern C_ChatInfo API if available, fallback to classic
...
local function SendChatOrPrint(msg)
    ...
    -- Try primary channel -> Fallback 1 -> Fallback 2
...
local LEGACY_IMPORT_KEY_MAP = {
    -- Old non-prefixed keys to new stealth-prefixed keys
...
function ns.DeleteProfile(name)
    ...
    if db.profileKeys and db.profileKeys[charKey] == name then
        db.profileKeys[charKey] = GetCharacterKey()
        ns.activeProfileName = db.profileKeys[charKey]
        ns.db = db.profiles[ns.activeProfileName]
    end
    return true
end
...
function ns.ApplyHighlight()
    ...
    if not sd then enableHL = db.stealthEnableHighlight end
    local hlType = sd and db.sdHighlightType or db.stealthHighlightType
